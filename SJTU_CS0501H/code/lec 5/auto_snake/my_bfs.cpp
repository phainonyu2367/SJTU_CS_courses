...
void _enqueueX(int x) {
    bfsQueueX[Xrear] = x;
    Xrear = (Xrear + 1) % max;
}

void _enqueueY(int y) {
    bfsQueueY[Yrear] = y;
    Yrear = (Yrear + 1) % max;
}
...
int _dequeueX() {
    Xfront = (Xfront + 1) % max;
    return bfsQueueX[Xfront];
}

int _dequeueY() {

    Yfront = (Yfront + 1) % max;
    return bfsQueueY[Yfront];
}

bool _isEmpty() {
    return (Xfront + 1) % max == Xrear;
}
...
bool _checkAvailability(int x, int y) {
    int cell = getMapCell(x, y);
    if (cell == 0 || cell == 4) return true;
    return false;
}

bool _checkSafety(int x, int y) {
    int cell = getMapCell(x, y);
...
int _bfs(int x, int y) {
    // find the best dir with BFS
    _resetBFS();

    _enqueueX(x); _enqueueY(y); // 初始蛇头入队列
    visited[30 * y + x] = 1;
    
    int curX = 0; int curY = 0;
    int foodX = getFoodX(); int foodY = getFoodY();
    int bestDir = 0;

    while (!_isEmpty()) {
        curX = _dequeueX(); curY = _dequeueY();
        for (int i = 0; i < 4; i++) {
            int nextX = curX + dx[i];
            int nextY = curY + dy[i];

            if (_checkAvailability(nextX, nextY) && visited[30 * nextY + nextX] == 0) {
                _enqueueX(nextX);
                _enqueueY(nextY);
                visited[30 * nextY + nextX] = 1;
                parentDir[30 * nextY + nextX] = i;

                if (nextX == foodX && nextY == foodY) {
                    bestDir = i;
                    while (nextX - dx[bestDir] != x || nextY - dy[bestDir] != y) {
                        nextX -= dx[bestDir];
                        nextY -= dy[bestDir];
                        bestDir = parentDir[30 * nextY + nextX];
                    }
                    return bestDir;
                }
            }
...
int decideDirection() {
    ...
    int bestDir = _bfs(headX, headY); // 通过bfs算法查找最优方向

    if (!_checkSafety(headX + dx[bestDir], headY + dy[bestDir])) { 
        // 最后检查，如果bfs因为特殊情况给出了错误方向，那么随机选择一个安全的方向
        for (int i = 0; i < 4; i++) {
            if (_checkSafety(headX + dx[i], headY + dy[i])) return i;
        }
    }
    return bestDir;
}